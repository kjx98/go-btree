...
func testMapNewBTree() *Map[testMapKind, testMapKind] {
	return New[testMapKind, testMapKind]()
}
...
func mapKindsAreEqual(a, b []testMapKind) bool {
	tr := MapNew[testMapKind, testMapKind](testMapMapLess)
	if len(a) != len(b) {
		return false
	}
	for i := 0; i < len(a); i++ {
...
func BenchmarkMapInsertSeq(b *testing.B) {
	b.StopTimer()
	tree := MapNew[testMapKind, int](func(a, b testMapKind) bool {
		return a < b
	})
	for i := 0; i < 1e6; i++ {
...
func BenchmarkMapInsertRandom(b *testing.B) {
	b.StopTimer()
	tree := MapNew[testMapKind, int](testMapMapLess)
	for i := 0; i < 1e6; i++ {
		tree.Set(testMapKind(i), i)
	}
	b.StartTimer()
